...
// Mock data (can be moved to a shared file)
const mockProducts = [
...
function QuotationForm({ onSave, onCancel, initialData = null }) {
  const [formData, setFormData] = useState({
    id: initialData?.id || Date.now(),
    quotationNumber: initialData?.quotationNumber || `QUO-${new Date().getFullYear()}-${String(Math.floor(Math.random() * 1000)).padStart(3, '0')}`,

...
export default function ShopSetupChecklistForm() {
  ...
  const validateDate = (dateStr) => {
    ...
    if (!dateRegex.test(dateStr)) {
      return (
        validationMessages[language]?.invalidDateFormat || "Invalid date format"
      );
    }
    const inputDate = new Date(dateStr);
    const today = new Date();
    if (isNaN(inputDate.getTime())) {
      return validationMessages[language]?.invalidDate || "Invalid date";
    }
    if (inputDate < today.setHours(0, 0, 0, 0)) {
      return (
        validationMessages[language]?.pastDate || "Date cannot be in the past"
      );
    }
    return null;
  };
  ...
  const validateCurrentField = (field) => {
    ...
    if (field.type === "file" && field.required && !formData[id]) {
      newErrors[id] =
        validationMessages[language]?.imageRequired || "प्रतिमा आवश्यक आहे";
    }
    ...
    if (field.followup && formData[id] !== undefined) {
      ...
      followup.forEach((subField) => {
        ...
        if (
          subField.type === "text" &&
          subField.id?.toLowerCase().includes("date") &&
          formData[subField.id]
        ) {
          const error = validateDate(formData[subField.id]);
          if (error) newErrors[subField.id] = error;
        }
        // Nested followup for radio
        if (
          subField.type === "radio" &&
          subField.options &&
          formData[subField.id]
          ...
          if (selectedOption?.followup?.fields) {
            selectedOption.followup.fields.forEach((nestedField) => {
              ...
              if (
                ...
                nestedField.id?.toLowerCase().includes("date") &&
                formData[nestedField.id]
              ) {
  ...
  const handleTextChange = (id, value) => {
    ...
    setErrors((prev) => ({
      ...prev,
      [id]: id.toLowerCase().includes("date") ? validateDate(value) : null,
    }));
  };
        ...
        {field.type === "text" && (
          <div>
            <input
              ...
              name={id}
              placeholder={
                field[`placeholder_${language}`] ||
                field.placeholder_mr ||
            ...
            <div className="mt-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
              <p className="text-gray-700 mb-2">
                {field.followup[formData[id] ? "yes" : "no"]?.[
                  ...
                  field.followup[formData[id] ? "yes" : "no"]?.message_mr ||
                  field.followup[formData[id] ? "yes" : "no"]?.message_en}
              </p>

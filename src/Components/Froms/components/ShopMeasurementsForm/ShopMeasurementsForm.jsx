...
export default function ShopMeasurementsForm() {
  ...
  const handleInputChange = (fieldId, value) => {
    ...
    const newErrors = validateField(subfield, fieldId, value);
  ...
  const validateForm = () => {
    ...
    currentField.subfields.forEach((subfield) => {
      ...
      if (subfield.required && !formData[fieldId]?.trim()) {
        ...
        subfield.type === "text" &&
        formData[fieldId] &&
        !/^[a-zA-Z\s]+$/.test(formData[fieldId].trim())
      ) {
        newErrors[fieldId] = validationMessages[language].invalidDateFormat;
      }
    });
    return newErrors;
  };
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-wrap gap-4">
          {field.subfields?.map((subfield) => {
            if (subfield.id === "total_sqft") return null;
